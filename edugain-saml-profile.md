...
| Entity | Entity means an AAI endpoint. For example, an Entity can be an Identity Provider, a Service Provider or an Attribute Provider. In this document, an Entity refers to an entity’s metadata that a Participant Federation has publishedexchanged through eduGAIN. |
